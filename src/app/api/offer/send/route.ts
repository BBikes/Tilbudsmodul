...
export async function POST(req: Request) {
  ...
  // Fetch the ticket from BikeDesk as early as possible so we have the assignee
  let ticket;
  try {
  ...
  let commentUserId: number | null = body.mechanicBikedeskUserId || ticket?.assignee || null;

  if (!commentUserId) {
    try {
      commentUserId = getBikedeskApiUserId();
    } catch (err) {
      console.error('[offer/send] Invalid API user id', err);
    }
  }
